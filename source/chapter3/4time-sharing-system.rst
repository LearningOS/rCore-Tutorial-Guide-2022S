...
实现调度算法需要计时，RISC-V 要求处理器有一个内置时钟，运行在 M 特权级的 SEE 已经预留了相应的接口，我们可以调用它们来间接控制计时器：
...
为了避免 S 特权级时钟中断被屏蔽，我们需要在执行第一个应用前调用 ``enable_timer_interrupt()`` 设置 ``sie.stie``，使得 S 特权级时钟中断不会被屏蔽；
再设置第一个 10ms 的计时器。
...
在用户程序中，三个 ``power`` 系列应用并没有主动 yield ，内核负责公平地把时间片分配给它们。
