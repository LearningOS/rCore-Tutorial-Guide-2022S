...
本节讲解如何实现为批处理系统实现用户态应用程序，要点是：

- 应用程序的内存布局；
- 应用程序发出的系统调用，即应用程序如何通过操作系统提供的服务完成自身的任务。
...
.. attention::
    ...
    用户库看起来很复杂，因为它预留了直到 ch7 内核才能实现的系统调用接口！你只需关注本节提到的部分即可。
...
方便我们在用户库链接脚本中将它作为用户程序的入口。

而从第 4 行开始，我们手动清零 ``.bss`` 段，然后调用 ``main`` 函数得到一个类型为 ``i32`` 的返回值，
最后，调用用户库提供的 ``exit`` 接口退出，并将这个返回值告知批处理系统。
...
这个接口被称为 ABI 或者系统调用。
现在我们不关心 S 态的批处理系统如何提供应用程序所需的功能，只是使用它即可。
...
用户程序会从 ``ecall`` 的下一条指令继续执行，同时能在合适的寄存器中读取返回值。
...
传入到 ``write`` 的 ``fd`` 参数设置为 1，代表标准输出，我们暂时不需要考虑其他的 ``fd`` 选取情况。
