...
为了做到这一步，首先需要让程序不依赖于标准库，并通过编译。这要求我们编写少量代码代替标准库的功能。见 :ref:`移除标准库依赖 <term-remove-std>`。

接下来要让脱离了标准库的程序能输出（即支持 ``println!``)，这对程序的开发和调试至关重要。我们先在用户态下实现该功能，需要利用内嵌汇编发起简单系统调用。见 :ref:`构建用户态执行环境 <term-print-userminienv>`。

最后要把程序移植到内核态，构建在裸机上支持输出的最小运行时环境。我们需要考虑更多的细节，包括如何关机，如何配置物理内存空间，如何通过 ``RustSBI`` 提供的 ``SBI_CONSOLE_PUTCHAR`` 接口实现输出。我们需要编写更多的底层代码。见 :ref:`构建裸机执行环境 <term-print-kernelminienv>`。

.. attention::
   实验指导书存在的目的是帮助读者理解框架代码。为便于测试，完成编程实验时，请以框架代码为基础，不必跟着文档从零开始编写内核。
