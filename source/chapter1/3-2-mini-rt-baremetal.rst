...
本节中，我们将把 ``Hello world!`` 应用程序从用户态态搬到内核态。 
...
从第 5 行开始体现了链接过程中对输入的目标文件的段的合并。其中 ``.`` 表示当前地址。
最终， 代码段 ``.text``, 只读数据段 ``.rodata``，数据段 ``.data``, bss 段 ``.bss`` 由低到高依次放置，
...
在这段汇编代码中，我们从第 8 行开始预留了一块大小为 4096 * 16 字节，也就是 :math:`64\text{KiB}` 的空间，
用作操作系统的栈空间，
栈顶地址被全局符号 ``boot_stack_top`` 标识，栈底则被全局符号 ``boot_stack`` 标识。
同时，这块栈空间单独作为一个名为 
``.bss.stack`` 的段，链接脚本里有它的位置。

从第 2 行开始，我们通过汇编代码实现执行环境的初始化，只有两条指令：
第一条指令将 sp 设置为我们预留的栈空间的栈顶位置，于是之后在函数
调用的时候，栈就可以从这里开始向低地址增长了。
简单起见，我们目前暂时不考虑 sp 越过了栈底 ``boot_stack`` ，也就是栈溢出的情形。虽然这有
可能导致严重的错误。第二条指令则是函数调用 ``rust_main`` ，这里的 ``rust_main`` 是我们稍后自己编写的应用
...
从第 10 行开始，
我们声明了应用的入口点 ``rust_main`` ，需要注意的是，这里通过宏将 ``rust_main`` 标记为 ``#[no_mangle]`` 
...
等一等，与内存相关的部分太容易出错了， **清零 .bss段** 的工作我们还没有完成。
...
通过链接脚本 ``linker.ld`` 中给出的全局符号 ``sbss`` 和 ``ebss``
让我们能轻松确定 ``.bss`` 段的位置。
